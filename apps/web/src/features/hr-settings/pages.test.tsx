...
describe("HrSyncSettingsPage", () => {
  ...
  it("loads and renders impact reviews", async () => {
    ...
    expect(within(row as HTMLTableRowElement).getByText("open")).toBeInTheDocument();
  });
  ...
  it("resolves an impact review with operator reason", async () => {
    ...
    const reasonInput = await screen.findByRole("textbox", { name: /resolve reason/i });
    await userEvent.clear(reasonInput);
    await userEvent.type(reasonInput, "approved by HR");
    await userEvent.click(screen.getByRole("button", { name: "Resolve" }));

    await waitFor(() => expect(resolveEligibilityImpactReview).toHaveBeenCalledWith("rev-2", { reason: "approved by HR" }));
    expect(screen.getByText("已解析 review rev-2。")).toBeInTheDocument();
    await waitFor(() => expect(screen.queryByText("目前沒有影響項目")).toBeInTheDocument());
