...
**Diagram: Sales rank high-level design**

```mermaid
flowchart LR
  Order[Order placed] --> App[Commerce app]
  App --> DB[(Orders DB)]
  DB --> Batch[Rank batch job]
  Batch --> Rank[(Sales rank table)]
  Rank --> API[Product API]
```
...
The **Sales Rank Service** could use **MapReduce**, using the **Sales API** server log files as input and writing the results to an aggregate table `sales_rank` in a **SQL Database**.  We should discuss the [use cases and tradeoffs between choosing SQL or NoSQL](../../../primer.md#sql-or-nosql).
...
**Diagram: Sales rank scaled design**

```mermaid
flowchart LR
  Order[Order events] --> Queue[Event queue]
  Queue --> Stream[Stream processors]
  Stream --> RankStore[(Rank store)]
  Queue --> Batch[Periodic recomputation]
  Batch --> RankStore
  RankStore --> Cache[Rank cache]
  Cache --> API[Product API]
```
