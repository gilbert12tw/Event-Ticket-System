...
**Diagram: Mint high-level design**

```mermaid
flowchart LR
  User[User] --> Web[Web app]
  Web --> App[Mint application]
  App --> Auth[Auth]
  App --> Aggregator[Bank aggregation service]
  Aggregator --> Banks[Bank APIs]
  App --> DB[(Transactions DB)]
...
We could store info on the 10 million users in a [relational database](../../../primer.md#relational-database-management-system-rdbms).  We should discuss the [use cases and tradeoffs between choosing SQL or NoSQL](../../../primer.md#sql-or-nosql).
...
* The **Accounts API** server places a job on a **Queue** such as [Amazon SQS](https://aws.amazon.com/sqs/) or [RabbitMQ](https://www.rabbitmq.com/)
    * Extracting transactions could take awhile, we'd probably want to do this [asynchronously with a queue](../../../primer.md#asynchronism), although this introduces additional complexity
...
We'll create an [index](../../../primer.md#use-good-indices) on `id`, `user_id `, and `created_at`.
...
We'll create an [index](../../../primer.md#use-good-indices) on `id` and `user_id `.
...
flowchart LR
  ...
  API --> Queue[Sync queue]
  Queue --> Workers[Aggregation workers]
  Workers --> Banks[Bank APIs]
  Workers --> Analytics[Budget analytics]
```
...
* [Asynchronism](../../../primer.md#asynchronism)
