...
**Diagram: Initial single-node design**

```mermaid
flowchart LR
  Users[Users] --> Server[Single web and app server]
...
* **Web server** on EC2
    ...
    * [**MySQL Database**](../../../primer.md#relational-database-management-system-rdbms)
...
* The alternative to **Vertical Scaling** is [**Horizontal scaling**](../../../primer.md#horizontal-scaling)
...
* See the [Relational database management system (RDBMS)](../../../primer.md#relational-database-management-system-rdbms) section
* Discuss reasons to use [SQL or NoSQL](../../../primer.md#sql-or-nosql)
...
* See the [Domain name system](../../../primer.md#domain-name-system) section
...
* See the [Security](../../../primer.md#security) section
...
**Diagram: Users+: separate database and object storage**

```mermaid
flowchart LR
  Users[Users] --> Web[Web app server]
  Web --> DB[(Managed database)]
...
**Diagram: Users++: add load balancing and redundancy**
...
* Use [**Horizontal Scaling**](../../../primer.md#horizontal-scaling) to handle increasing loads and to address single points of failure
    * Add a [**Load Balancer**](../../../primer.md#load-balancer) such as Amazon's ELB or HAProxy
        * ELB is highly available
        * If you are configuring your own **Load Balancer**, setting up multiple servers in [active-active](../../../primer.md#active-active) or [active-passive](../../../primer.md#active-passive) in multiple availability zones will improve availability
        * Terminate SSL on the **Load Balancer** to reduce computational load on backend servers and to simplify certificate administration
    * Use multiple **Web Servers** spread out over multiple availability zones
    * Use multiple **MySQL** instances in [**Master-Slave Failover**](../../../primer.md#master-slave-replication) mode across multiple availability zones to improve redundancy
* Separate out the **Web Servers** from the [**Application Servers**](../../../primer.md#application-layer)
    * Scale and configure both layers independently
    * **Web Servers** can run as a [**Reverse Proxy**](../../../primer.md#reverse-proxy-web-server)
    * For example, you can add **Application Servers** handling **Read APIs** while others handle **Write APIs**
* Move static (and some dynamic) content to a [**Content Delivery Network (CDN)**](../../../primer.md#content-delivery-network) such as CloudFront to reduce load and latency
...
**Diagram: Users+++: cache and read scaling**
...
* The following goals attempt to address the scaling issues with the **MySQL Database**
    * Based on the **Benchmarks/Load Tests** and **Profiling**, you might only need to implement one or two of these techniques
* Move the following data to a [**Memory Cache**](../../../primer.md#cache) such as Elasticache to reduce load and latency:
    * Frequently accessed content from **MySQL**
        * First, try to configure the **MySQL Database** cache to see if that is sufficient to relieve the bottleneck before implementing a **Memory Cache**
    * Session data from the **Web Servers**
        * The **Web Servers** become stateless, allowing for **Autoscaling**
    * Reading 1 MB sequentially from memory takes about 250 microseconds, while reading from SSD takes 4x and from disk takes 80x longer.<sup><a href=../../../primer.md#latency-numbers-every-programmer-should-know>1</a></sup>
* Add [**MySQL Read Replicas**](../../../primer.md#master-slave-replication) to reduce load on the write master
...
**Diagram: Users++++: autoscaling app tier**

```mermaid
flowchart LR
  Users[Users] --> LB[Load balancer]
  LB --> ASG[Autoscaling web/app group]
  ASG --> Cache[Cache]
  ASG --> DB[(Primary DB)]
  DB --> Replicas[(Read replicas)]
  ASG --> Queue[Job queue]
  Queue --> Workers[Autoscaling workers]
```
...
**Diagram: Users+++++: multi-AZ production architecture**

```mermaid
flowchart LR
  Users[Users] --> CDN[CDN]
  CDN --> LB[Public load balancer]
  LB --> AZ1[App tier AZ-1]
  LB --> AZ2[App tier AZ-2]
  AZ1 --> Cache[(Distributed cache)]
  AZ2 --> Cache
  AZ1 --> DB[(Primary DB)]
  AZ2 --> Replica[(Standby or read replica)]
  DB --> Replica
  AZ1 --> Queue[Managed queue]
  AZ2 --> Queue
  Queue --> Workers[Worker tier]
  AZ1 --> Store[(Object storage)]
  AZ2 --> Store
```
...
To further address the high read and write requests, we should also consider moving appropriate data to a [**NoSQL Database**](../../../primer.md#nosql) such as DynamoDB.

We can further separate out our [**Application Servers**](../../../primer.md#application-layer) to allow for independent scaling.  Batch processes or computations that do not need to be done in real-time can be done [**Asynchronously**](../../../primer.md#asynchronism) with **Queues** and **Workers**:
