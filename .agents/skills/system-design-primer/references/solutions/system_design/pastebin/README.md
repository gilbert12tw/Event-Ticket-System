...
**Diagram: Pastebin high-level design**
...
We could use a [relational database](../../../primer.md#relational-database-management-system-rdbms) as a large hash table, mapping the generated url to a file server and path containing the paste file.

Instead of managing a file server, we could use a managed **Object Store** such as Amazon S3 or a [NoSQL document store](../../../primer.md#document-store).

An alternative to a relational database acting as a large hash table, we could use a [NoSQL key-value store](../../../primer.md#key-value-store).  We should discuss the [tradeoffs between choosing SQL or NoSQL](../../../primer.md#sql-or-nosql).  The following discussion uses the relational database approach.

* The **Client** sends a create paste request to the **Web Server**, running as a [reverse proxy](../../../primer.md#reverse-proxy-web-server)
...
We'll use a public [**REST API**](../../../primer.md#representational-state-transfer-rest):
...
For internal communications, we could use [Remote Procedure Calls](../../../primer.md#remote-procedure-call-rpc).
...
**Diagram: Pastebin scaled design**
...
flowchart LR
  ...
  App --> Queue[Expiration queue]
  Queue --> Worker[Cleanup worker]
```
...
* [CDN](../../../primer.md#content-delivery-network)
...
* [Relational database management system (RDBMS)](../../../primer.md#relational-database-management-system-rdbms)
* [SQL write master-slave failover](../../../primer.md#fail-over)
* [Master-slave replication](../../../primer.md#master-slave-replication)
