...
**Diagram: Web crawler high-level design**

```mermaid
flowchart LR
  Seed[Seed URLs] --> Frontier[URL frontier]
  Frontier --> Crawler[Crawler]
  Crawler --> Fetch[Fetch page]
  Fetch --> Parser[Parser]
...
**Diagram: Web crawler scaled design**

```mermaid
flowchart LR
  Frontier[Distributed URL frontier] --> Scheduler[Scheduler]
  Scheduler --> Worker1[Crawler worker]
  Scheduler --> Worker2[Crawler worker]
  Worker1 --> DNS[DNS cache]
  Worker2 --> DNS
  Worker1 --> Fetch[Fetch pages]
  Worker2 --> Fetch
  Fetch --> Parser[Parser]
...
* The **Crawler Service** can improve performance and reduce memory usage by keeping many open connections at a time, referred to as [connection pooling](https://en.wikipedia.org/wiki/Connection_pool)
    * Switching to [UDP](../../../primer.md#user-datagram-protocol-udp) could also boost performance
