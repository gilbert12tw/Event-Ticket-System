...
kubectl_bm create namespace cnpg-system --dry-run=client -o yaml | kubectl_bm apply -f -
kubectl_bm create namespace observability --dry-run=client -o yaml | kubectl_bm apply -f -

log "installing local-path default StorageClass"
kubectl_bm apply -f https://raw.githubusercontent.com/rancher/local-path-provisioner/v0.0.36/deploy/local-path-storage.yaml
kubectl_bm annotate storageclass local-path storageclass.kubernetes.io/is-default-class=true --overwrite
kubectl_bm rollout status deployment/local-path-provisioner -n local-path-storage --timeout=180s
...
helm upgrade --install cloudnative-pg cnpg/cloudnative-pg \
  ...
  --create-namespace \
  --version 0.28.2
kubectl_bm rollout status deployment/cloudnative-pg -n cnpg-system --timeout=180s

log "writing app secrets"
...
kubectl_bm -n "$CETS_NAMESPACE" delete job minio-init --ignore-not-found
...
kubectl_bm apply -f "$GENERATED_DIR/cets-platform.yaml"
kubectl_bm wait --for=condition=Ready cluster/cets-postgres -n "$CETS_NAMESPACE" --timeout=600s
kubectl_bm wait --for=condition=complete job/minio-init -n "$CETS_NAMESPACE" --timeout=300s
}
