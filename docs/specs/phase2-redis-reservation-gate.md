...
- Lottery bookings that create `received` registrations do not create Redis holds, because capacity is consumed only when the lottery run confirms winners after cutoff.
- This spec does not change Phase 1 family-count, cooldown, cutoff, or waitlist semantics.
