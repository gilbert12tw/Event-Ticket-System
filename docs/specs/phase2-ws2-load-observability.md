...
- Tempo exposes local OTLP gRPC/HTTP receivers for trace ingestion. The app can opt in to route-bounded HTTP server spans with `OTEL_TRACES_ENABLED=true`; the default remains off so product traffic does not depend on the trace backend.
...
- Trace backend: Tempo is optional Compose service only. App OTLP export is opt-in via typed env config and defaults off; rollback is setting `OTEL_TRACES_ENABLED=false` or removing the observability profile.
