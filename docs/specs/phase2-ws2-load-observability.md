...
- `services/api/deploy/reference/kubernetes-observability/` keeps reference-only kube-state-metrics and Prometheus scrape examples for orchestration-state visibility in a future container-platform decision gate. The examples are not loaded by the local Compose profile and do not change app, worker, booking, check-in, reporting, or audit behavior.
