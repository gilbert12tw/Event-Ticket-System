...
- `services/api/deploy/reference/kubernetes-observability/` keeps reference-only Kubernetes observability examples for orchestration-state, container metrics, container log pipeline visibility, and application lifecycle/networking automation in a future container-platform decision gate. They cover kube-state-metrics, node exporter as a DaemonSet, app-specific exporter and Fluent Bit log sidecars, Prometheus and Loki as StatefulSets with persistent storage, Grafana as a stateless Deployment, Elasticsearch/Kibana EFK examples, scrape examples, rolling updates, health probes, HPA scaling, stable Service names, PodDisruptionBudget, and node-spread hints. The examples are not loaded by the local Compose profile and do not change app, worker, booking, check-in, reporting, or audit behavior.
