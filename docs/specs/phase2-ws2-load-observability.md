...
- `services/api/deploy/reference/kubernetes-observability/` keeps reference-only Kubernetes observability examples for orchestration-state, container metrics, and container log pipeline visibility in a future container-platform decision gate. They cover kube-state-metrics, node exporter as a DaemonSet, app-specific exporter and Fluent Bit log sidecars, Prometheus and Loki as StatefulSets with persistent storage, Grafana as a stateless Deployment, Elasticsearch/Kibana EFK examples, and scrape examples. The examples are not loaded by the local Compose profile and do not change app, worker, booking, check-in, reporting, or audit behavior.
