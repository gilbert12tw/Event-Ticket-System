...
- `services/api/deploy/reference/kubernetes-observability/` keeps reference-only Kubernetes observability examples for orchestration-state and container metrics visibility in a future container-platform decision gate. They cover kube-state-metrics, node exporter as a DaemonSet, an app-specific exporter sidecar, Prometheus as a StatefulSet with persistent metric storage, Grafana as a stateless Deployment, and scrape examples. The examples are not loaded by the local Compose profile and do not change app, worker, booking, check-in, reporting, or audit behavior.
