...
- `services/api/deploy/reference/kubernetes-observability/` contains reference-only Kubernetes observability examples for a future container-platform decision gate: kube-state-metrics, node exporter as a DaemonSet, an app-specific exporter sidecar, Prometheus as a StatefulSet with a persistent volume claim, Grafana as a stateless Deployment, and Prometheus scrape examples. These files are not active deployment assets, are not loaded by Compose, and must not become product runtime dependencies.
