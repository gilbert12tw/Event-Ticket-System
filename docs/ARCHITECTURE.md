...
- The optional Compose `observability` profile starts Prometheus, Grafana, and blackbox exporter from `services/api/deploy/observability/` so reviewers can inspect RED, DB pool/lock, outbox lag, and HTTP boundary probe signals without adding a required production backing service. Black-box probes target `/`, `/healthz`, and `/readyz`; they do not call product APIs or booking/check-in flows.
