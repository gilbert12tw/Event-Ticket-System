...
The optional local observability profile also provisions Loki for app/worker stdout log search, Tempo as a local trace backend, and node exporter / cAdvisor for host and container USE metrics. This does not change app/worker runtime behavior: logs still go to stdout/stderr, trace export remains a future opt-in app config change, and infra exporters are scrape-only review/demo services.
