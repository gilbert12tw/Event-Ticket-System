...
- The optional Compose `observability` profile starts Prometheus and Grafana from `services/api/deploy/observability/` so reviewers can inspect RED, DB pool/lock, and outbox lag panels without adding a required production backing service. Prometheus also loads starter SLO alert rules for HTTP 5xx error rate, P99 latency, DB pool acquire wait, DB lock waits, outbox lag, and metrics scrape failures; these rules are review/demo evidence until an Alertmanager or production paging route is added.
