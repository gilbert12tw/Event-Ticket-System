...
- `services/api/deploy/reference/kubernetes-observability/` contains reference-only Kubernetes observability examples for a future container-platform decision gate: kube-state-metrics, node exporter as a DaemonSet, app-specific exporter and Fluent Bit log sidecars, Prometheus and Loki StatefulSets with persistent volume claims, Grafana as a stateless Deployment, Elasticsearch/Kibana EFK examples, Prometheus scrape examples, and app lifecycle/service-discovery examples for rolling updates, health probes, HPA scaling, stable Service names, PodDisruptionBudget, and node spread. These files are not active deployment assets, are not loaded by Compose, and must not become product runtime dependencies.
