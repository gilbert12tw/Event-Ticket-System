...
- `services/api/deploy/reference/kubernetes-observability/` contains reference-only Kubernetes observability examples for a future container-platform decision gate: kube-state-metrics, node exporter as a DaemonSet, app-specific exporter, MySQL exporter database-metrics translation, Alertmanager local-review and PagerDuty-style receiver routing, Fluent Bit / Fluentd / Vector log collector examples, unstructured / logfmt / Common Log Format / structured JSON parser examples, Prometheus and Loki StatefulSets with persistent volume claims, Grafana as a stateless Deployment, Thanos Receive / Cortex / Grafana Agent remote-write alternatives for long-term metrics storage, Elasticsearch/Kibana EFK examples, Logstash/OpenSearch log analysis alternatives, Prometheus scrape examples, app lifecycle/service-discovery examples for rolling updates, health probes, HPA scaling, stable Service names, PodDisruptionBudget, and node spread, plus OpenTelemetry Collector, Tempo, Jaeger, and Zipkin trace pipeline examples. These files are not active deployment assets, are not loaded by Compose, and must not become product runtime dependencies.
