...
func TestLoadDefaultsAndEnv(t *testing.T) {
	...
	t.Setenv("BOOKING_RATE_LIMIT_RPS_PER_ACTOR", "3")
	t.Setenv("BOOKING_RATE_LIMIT_RPS_PER_EVENT", "35")
	t.Setenv("RATE_LIMIT_OUTAGE_MODE", "fail")
	t.Setenv("BOOKING_RATE_LIMIT_HASH_SECRET", "rate-limit-hash-secret")
	t.Setenv("BOOKING_PREADMISSION", "on")
	t.Setenv("BOOKING_CONTENTION_STRATEGY", "advisory")
	...
	assert.Equal(t, 3, cfg.BookingRateLimitPerActor)
	assert.Equal(t, 35, cfg.BookingRateLimitPerEvent)
	assert.Equal(t, "fail", cfg.RateLimitOutageMode)
	assert.Equal(t, "rate-limit-hash-secret", cfg.BookingRateLimitHashSecret)
	assert.True(t, cfg.BookingPreadmission)
	assert.Equal(t, "advisory", cfg.BookingContentionStrategy)
