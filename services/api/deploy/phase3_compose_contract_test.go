...
func TestPhase3ComposeScriptsDeclareDeployVerifyAndDrillContracts(t *testing.T) {
	...
	for _, fragment := range []string{
		...
		"tags=service.name=cets-backend",
		"generate_fresh_traffic",
		"VERIFY_START_SECONDS",
		"/api/v1/phase3-verify/",
		"check_run_scoped_backend_logs",
		"traces_service_graph_request_total[2m]",
		"pyroscope/render",
		"now-5m",
		"otel_trace_id",
		"phase3-redaction-canary",
		`{service_name="redaction-canary"} |= "phase3-redaction-canary"`,
