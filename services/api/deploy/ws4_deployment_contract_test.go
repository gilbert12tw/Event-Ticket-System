...
func TestLiveGatesRunWorkerIsolationLagK6Gate(t *testing.T) {
	...
	assert.Contains(t, workflowContent, "K6_REQUIRED_WORKER_KINDS=export,projection")
	...
	dockerActAction, err := os.ReadFile("../../../.github/actions/setup-docker-act/action.yml")
	require.NoError(t, err)
	dockerActContent := string(dockerActAction)
	assert.Contains(t, dockerActContent, "docker-compose-v2")
	assert.Contains(t, dockerActContent, "docker compose version")
...
func TestSonarCoverageConfigStaysLocalOnly(t *testing.T) {
	...
	assert.Contains(t, workflowContent, `".codex/**"`,
		"local Codex artifacts must start CI so detect-changes can reject reintroduction")
	...
	assert.Contains(t, detectorContent, `(goal|note)\.md|\.codex/`)
	assert.Contains(t, detectorContent, `goal.md, note.md, and .codex/ are local agent artifacts`)
