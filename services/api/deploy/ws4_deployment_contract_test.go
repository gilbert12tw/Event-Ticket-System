...
func TestLiveGatesRunWorkerIsolationLagK6Gate(t *testing.T) {
	...
	assert.Contains(t, ciGateContent, `services/api/cmd/cets/worker.*\.go`)
	assert.Contains(t, ciGateContent, "services/api/internal/config/")
	assert.Contains(t, ciGateContent, `services/api/internal/postgres/schema\.sql`)
	assert.Contains(t, ciGateContent, `services/api/internal/ticketing/(worker|outbox|notification).*\.go`)
	assert.Contains(t, ciGateContent, "services/api/internal/observability/")
